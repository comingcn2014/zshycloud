...
public class SysApiController extends BaseController {
	...
	public Result<?> sync() {
		Set<Object> serviceIds = redisService.sGet(ZshyConstant.ZSHY_SERVICE_RESOURCE);
		for (Object service : serviceIds) {
			if (redisService.hHasKey(ZshyConstant.ZSHY_API_RESOURCE, service.toString())) {
				Map<String, Object> apiMap = (Map<String, Object>) redisService.hget(ZshyConstant.ZSHY_API_RESOURCE, service.toString());
			...
			redisService.hdel(ZshyConstant.ZSHY_API_RESOURCE, service.toString());
		}
		redisService.del(ZshyConstant.ZSHY_SERVICE_RESOURCE);
