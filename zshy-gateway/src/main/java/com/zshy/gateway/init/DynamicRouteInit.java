...
import com.zshy.gateway.entity.GatewayRoute;
...
public class DynamicRouteInit {
	...
	public void initRoute() {
		try {
			...
			String content = configService.getConfig(ZshyConstant.CONFIG_DATA_ID_DYNAMIC_ROUTES, nacosProperties.getGroup(), ZshyConstant.CONFIG_TIMEOUT_MS);
			log.info("初始化网关路由开始");
			updateRoute(content);
			log.info("初始化网关路由完成");
			//开户监听，实现动态
			configService.addListener(ZshyConstant.CONFIG_DATA_ID_DYNAMIC_ROUTES, nacosProperties.getGroup(), new Listener() {
				@Override
